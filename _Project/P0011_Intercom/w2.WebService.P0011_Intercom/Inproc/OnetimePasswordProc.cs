		...
		public DataSet Execute(DataSet ds)
		{
			...
			catch (Exception ex)
			{
				...
				if (rtnDS.Tables.Contains(WebServiceConst.TAB_NAME_RESULTTABLE))
				{
					...
					rtnDS.Tables[WebServiceConst.COL_PROCRESULT].Rows[0][WebServiceConst.COL_PROCMESSAGE] = "Failed";
					rtnDS.Tables[WebServiceConst.COL_PROCRESULT].Rows[0][WebServiceConst.COL_PROCMESSAGE] = "エラーが発生しました。エラーメッセージ→" + ex.Message;
				}
				else
				{
