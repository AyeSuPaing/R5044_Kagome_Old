...
var FixedMidashi = new function () {
  ...
  Nb.create = function () {
    ...
    var l = (new Date).getTime(),
      h = _fixedHeaders == Rb;
    if (!h)
      for (var b = 0; b < _fixedHeaders.length; b++) _fixedHeaders[b].removeAllTables(Ob);
  ...
  Nb.remove = function () {
    if (_fixedHeaders == Rb) return;
    for (var a = 0; a < _fixedHeaders.length; a++) _fixedHeaders[a].removeAllTables(Qb);
    _fixedHeaders = []
  };
  ...
  _resizeSourceDiv() {
    ...
    for (var a = 0; a < _fixedHeaders.length; a++) _fixedHeaders[a].hideAllDivs(Ob)
  }

  function
  _FixedHeader(a, d, E, p, y, v, B, xb, Z, F, G, J, K, L, zb, Q, bb, M, N) {
    var o = "CLIENT",
      j = "TBODY",
      g = "1px",
      q = this,
    ...
    function Gb(l) {
      ...
      if (p > 0 && h)
        if (_rowsHeight(j, p) + 30 >= k.clientHeight) h = Ob;
      if (y > 0 && g) {
        var m = _cells(j[0]);
    ...
    wb() {
      ...
      a.width = Vb;
      a.height = Vb;
      var i, m = _getElementByTagName(d, Ub);
      if (m == Rb) m = _getElementByTagName(d, j);
      if (m != Rb) i = m.cloneNode(Ob);
      ...
      for (var c = 0; c < v; c++)
        for (var l = _cells(t[c]), w = _cells(i.rows[c]), f = 0; f < l.length; f++) {
          ...
          var g = e.currentStyle || document.defaultView.getComputedStyle(e, Vb),
            k = Sb;
          if (g.paddingLeft.match(/px$/) != Rb && g.paddingRight.match(/px$/) != Rb) k = _pixel(g.paddingLeft) + _pixel(g.paddingRight);
          else if (g.padding.match(/px$/) != Rb) k = _pixel(g.padding) * 2;
        ...
        for (var m = l[b], i = _cells(m), h = 0, a = 0; a < E;) {
          ...
          if (e.rowSpan >= 2 && e.colSpan >= 2)
            for (var c = 0; c < e.rowSpan; c++) {
              ...
              if (e.colSpan >= 2)
                for (var c = 1; c < e.colSpan; c++) f[b][a + c] = Ob
            }
          h++;
          a += e.colSpan
        }
      for (var k = new Array(E), a = 0; a < E; a++) k[a] = Ob;
      for (var b = 0; b < g; b++)
    ...
    pb(e, v, x) {
      ...
      if (f) {
        ...
        if (e == TID_HEADER) b.style.borderRightWidth = ac;
        else {
          b.style.marginRight = ac;
          b.style.borderRightWidth = ac;
          b.style.paddingRight = ac
        }
        if (e == TID_NUMBER) b.style.borderBottomWidth = ac;
        else {
    ...
    s(b) {
      ...
      var f = _rect(a),
        ...
        c = e.left - f.left;
      if (_isIE) {
        if (d == Sb && b.$TOP_DIFF == 1) d = 0;
        else b.$TOP_DIFF = d;
        if (c == Sb && b.$LEFT_DIFF == 1) c = 0;
        else b.$LEFT_DIFF = c
      }
  ...
  function
  _linkElement(a, b, k, i, c, g) {
    b.id && !i && a.removeAttribute("id");
    if (c) {
      var h = a.getAttribute(I);
      if (h) a.setAttribute(I, h + " " + c);
      else a.setAttribute(I, c)
    }
    if (b.name)
      if (b.tagName == l && b.type == m) a.name = RADIO_PREFIX + k + "_" + b.name;
      else a.removeAttribute("name");
    a.$SOURCE_ELEMENT = b;
    if (_fixedList != Rb) _fixedList.add(a);
    else if (!b.$FXH_FIXED_ELEMENT) b.$FXH_FIXED_ELEMENT = a;
    else {
  ...
  function
  _setStyle(element, styleName, value) {
    try {
  ...
  _addEventListener(a, b, c) {
    if (a.addEventListener) a.addEventListener(b, c, Ob);
    else a.attachEvent && a.attachEvent("on" + b, c)
  }

  function
  _removeEventListener(a, b, c) {
    if (a.removeEventListener) a.removeEventListener(b, c, Ob);
