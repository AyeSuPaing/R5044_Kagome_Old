...
public partial class Form_Operator_OperatorList : BasePage
{
	...
	private void DisplayList(Dictionary<string, string> paramData)
	{
		...
		var operators = new ShopOperatorService().GetEcOperatorList(htInputa, rowNumber);
	...
	/// ECメニュー権限名称取得
	/// </summary>
	/// <param name="ecOperator">ECオペレータ</param>
	/// <returns>メニュー権限表示文字列</returns>
	protected string GetEcMenuAccessLevelName(ShopOperatorModel ecOperator)
	{
		switch (ecOperator.EcMenuAccessLevel)
		{
			...
			default:
				return ecOperator.MenuAuthorityName;
		}
	}
	...
	public Hashtable CreateSearchParams()
	{
		var inputParam = new Hashtable
		{
			...
			{ "sort_kbn", this.ddlSortKbn.SelectedValue },
			{ "operator_id_like_escaped", this.tbOperatorId.Text.Trim() },
			{ "name_like_escaped", this.tbOperatorName.Text.Trim() },
			{ Constants.FIELD_SHOPOPERATOR_VALID_FLG, this.ddlValid.SelectedValue },
			{ "login_operator_menu_access_level1", this.LoginOperatorMenuAccessLevel },
			{ Constants.FIELD_SHOPOPERATOR_MENU_ACCESS_LEVEL1, this.ddlMenuAccess.SelectedValue },
			{ "delimiter", Constants.MASTERFILEIMPORT_DELIMITER }
		};
