...
<body>
	<div class="form-element-group-content" style="display: inline-block;">
		...
		<% } else { %>
			<span class="select-display-period-value-<%= this.ClientID %>" style="color: #b2b2b2; display: none;">設定なし</span>
...
<script>
	...
	var selectDateTimePeriodModal = {
		...
		startDateInputBox: '.select-period-start-input-date select-period-start-input-date-',
		...
		setDisableModal: function (ucControlId, isDisabled) {
			if (ucControlId === undefined) return;
			var _this = this;
			$(_this.wrapperSelector).each(function () {
				var wrapper = $(this);
				...
				var submitBtn = wrapper.find(_this.submitBtnSelector + controlId);
				if (controlId === ucControlId) {
					$(submitBtn.data('value-text-selector')).each(function () {
		...
		getValue: function (ucControlId) {
			var _this = this;
			var wrapper = $(_this.wrapperSelector + "[data-id = " + ucControlId + "]");
			var startDateInput = wrapper.find(_this.startDateInputSelector.replace('controlId', ucControlId));
			var startTimeInput = wrapper.find(_this.startTimeInputSelector.replace('controlId', ucControlId));
			var endDateInput = wrapper.find(_this.endDateInputSelector.replace('controlId', ucControlId));
			var endTimeInput = wrapper.find(_this.endTimeInputSelector.replace('controlId', ucControlId));

			var result = {
				valStartDate: $(startDateInput.data('input-hidden-selector')).val(),
				valStartTime: $(startTimeInput.data('input-hidden-selector')).val(),
				valEndDate: $(endDateInput.data('input-hidden-selector')).val(),
				valEndTime: $(endTimeInput.data('input-hidden-selector')).val()
			};

			return result;
		},
		setValue: function (ucControlId, startDate, startTime, endDate, endTime) {
			var _this = this;
			var wrapper = $(_this.wrapperSelector + "[data-id = " + ucControlId + "]");
			var startDateInput = wrapper.find(_this.startDateInputSelector.replace('controlId', ucControlId));
			var startTimeInput = wrapper.find(_this.startTimeInputSelector.replace('controlId', ucControlId));
			...
			$(startDateInput.data('input-hidden-selector')).val(startDate);
			$(startTimeInput.data('input-hidden-selector')).val(startTime);
			$(endDateInput.data('input-hidden-selector')).val(endDate);
			$(endTimeInput.data('input-hidden-selector')).val(endTime);

			_this.reload(ucControlId);
		}
	}
	$(function () {
	...
	// Set disable select period modal
	function setDisableSelectPeriodModal(ucControlId, isDisabled) {
		selectDateTimePeriodModal.setDisableModal(ucControlId, isDisabled);
	}

	// Reload init javascript function that missing when have post-back event (using for update panel)
	Sys.WebForms.PageRequestManager.getInstance().add_endRequest(function (evt, args) {
		var upControlId = '<%: this.UpdatePanelControlId %>';
		if (upControlId
			&& evt._postBackSettings.panelsToUpdate.length
			&& (evt._postBackSettings.panelsToUpdate[0].indexOf(upControlId) !== -1)) {
			modal.ini(true);
			selectPeriod.ini();
