...
function CallgetToken(ci, tokenHiddenID, paymentParams, isSameAs1Process) {
	...
	<%if (Constants.PAYMENT_CARD_KBN == w2.App.Common.Constants.PaymentCard.VeriTrans) { %>
		var data = {
			...
			card_number : ci.CardNo,
			card_expire : ci.ExpMM + "/" + ci.ExpYY,
			security_code : ci.SecurityCode,
			cardholder_name : ci.AuthorName,
			lang : "ja"
		};
	...
	<%if (Constants.PAYMENT_CARD_KBN == w2.App.Common.Constants.PaymentCard.Paygent) { %>
		...
		'<%= Constants.PAYMENT_PAYGENT_MERCHANTID %>',
		'<%= Constants.PAYMENT_PAYGENT_CREDIT_GENERATE_TOKEN_KEY %>',
		{
			card_number : ci.CardNo,
			expire_year : ci.ExpYY,
			expire_month : ci.ExpMM,
			cvc : ci.SecurityCode,
			name : ci.AuthorName
		}, getTokenCallbackPaygent);
...
<%if (Constants.PAYMENT_CARD_KBN == w2.App.Common.Constants.PaymentCard.VeriTrans) { %>
<%----------------------------------------------------
	ベリトランストークン取得後処理（コールバック）
------------------------------------------------------%>
function getTokenCallbackVeriTrans(result) {
	var resc = result.status;
	if (resc == "success") {
		$("#" + toHfID).val(result.token);
	} else {
		paymentErrorCount++;
	}
	tokenProcessingCountUntilGetTokenCount--;
...
<%if (Constants.PAYMENT_CARD_KBN == w2.App.Common.Constants.PaymentCard.EScott) { %>
<%----------------------------------------------------
	ソニーペイメントe-SCOTTトークン取得後処理（コールバック）
------------------------------------------------------%>
var setToken = function (token, card) {
		$("#" + toHfID).val(token);
	tokenProcessingCountUntilGetTokenCount--;
}
<%} %>
...
<%-- ▼▼ペイジェントトークンコールバック▼▼ --%>
<%if (Constants.PAYMENT_CARD_KBN == w2.App.Common.Constants.PaymentCard.Paygent) { %>
function getTokenCallbackPaygent(response) {
	var resCode = response.result;
	if (resCode == "0000") {
		<%-- トークンをセット --%>
		$("#" + toHfID).val(response.tokenizedCardObject.token);
	} else {
		paymentErrorCount++;
	}
	tokenProcessingCountUntilGetTokenCount--;
