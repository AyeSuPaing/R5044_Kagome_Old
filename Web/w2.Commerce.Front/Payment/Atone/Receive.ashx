...
public class Receive : IHttpHandler
{
	...
	public void ProcessRequest(HttpContext context)
	{
		try
		{
			...
			using (var reader = new StreamReader(context.Request.GetBufferedInputStream()))
			{
				...
				FileLogger.Write("AtoneApi", "Json Input: " + json);
			}
			...
			if (context.Request.HttpMethod != "POST")
			{
				...
				FileLogger.Write("AtoneApi", "Method Not Allowed");
				return;
			}
			
			// Check Header
			var header = context.Request.Headers["Atone-Confirmation-Checksum"];
			if (string.IsNullOrEmpty(header))
			{
				context.Response.StatusCode = 404;
				FileLogger.Write("AtoneApi", "No Header");
				return;
			}
			...
			if (body != null)
			{
				...
				{
					FileLogger.Write("AtoneApi", "Object Id Invalid");
					return;
				}
				...
				{
					FileLogger.Write("AtoneApi", "Object Id Valid");
					return;
				}

				SendMailToOperator(order, body.CardTranId);
				FileLogger.Write("AtoneApi", "OK");
				context.Response.StatusCode = 200;
			}
			else
			{
				FileLogger.Write("AtoneApi", "No Body");
	...
	private static void SendMailToOperator(OrderModel order, string cardTranId)
	{
		...
		body.AppendLine("Atone決済の不整合性が発生しました。");
