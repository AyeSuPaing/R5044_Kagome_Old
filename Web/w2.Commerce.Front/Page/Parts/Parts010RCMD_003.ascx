...
<dl>
<%if (this.DataKbn == "IC1") {%>
<dt><img src="<%= Constants.PATH_ROOT %>Contents/ImagesPkg/label_right_recommend.jpg" alt="おすすめ商品" /></dt>
<%} %>
<%if (this.DataKbn == "IC2") {%>
<dt></dt>
<%} %>
...
<a href="<%# WebSanitizer.UrlAttrHtmlEncode(CreateProductDetailUrl(Container.DataItem)) %>">
	<w2c:ProductImage ID="ProductImage1" ImageTagId="picture" ImageSize="<%# this.ImageSize %>" ProductMaster="<%# Container.DataItem %>" IsVariation="false" runat="server" /></a></li>
...
<%-- ▽商品セール価格有効▽ --%>
<span id="Span1" visible='<%# ProductPage.GetProductTimeSalesValid(Container.DataItem) %>' runat="server">
...
<%-- △商品セール価格有効△ --%>
<%-- ▽商品特別価格有効▽ --%>
<span id="Span2" visible='<%# ProductPage.GetProductSpecialPriceValid(Container.DataItem) %>' runat="server">
	<strike><%#: CurrencyManager.ToPrice(ProductPage.GetProductPriceNumeric(Container.DataItem)) %></strike>
	<%#: CurrencyManager.ToPrice(ProductPage.GetProductSpecialPriceNumeric(Container.DataItem)) %>
</span>
<%-- △商品特別価格有効△ --%>
<%-- ▽商品通常価格有効▽ --%>
<span id="Span3" visible='<%# ProductPage.GetProductNormalPriceValid(Container.DataItem) %>' runat="server">
