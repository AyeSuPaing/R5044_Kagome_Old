...
=========================================================================================================
--%>
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="PaymentYamatoKaSmsAuthModal.ascx.cs" Inherits="Form_Common_Order_PaymentYamatoKaSmsAuthModal" %>
<div id="modal">
	<div id="modal_content">
		<div id="authcodeinput">
			<p>電話番号<span id="spantelnum"><%: this.TelNum %></span>宛てにSMSを送信いたしました。</p>
			<p>SMSに記載の4桁の認証コードを入力してください。</p>
			<p>SMSが届かない場合は<a href="javascript:void(0)" class="btn_resend">こちら</a>をクリックし、電話番号を入力して下さい。</p>
			<p class="input_code"><asp:TextBox style="width:6em" TextMode="Number" min="0" MaxLength="4" placeholder="認証コード" Columns="6" ID="tbAuthCode" runat="server"/></p>
...
<script type="text/javascript">
	...
	function authorize(code, callback) {
		$.ajax({
			type: "POST",
			url: "<%=Constants.PATH_ROOT + Constants.PAGE_FRONT_ORDER_HISTORY_DETAIL%>/Authorize",
	...
	function smssend(tel, callback) {
		$.ajax({
			type: "POST",
			url: "<%=Constants.PATH_ROOT + Constants.PAGE_FRONT_ORDER_HISTORY_DETAIL%>/SmsSend",
			contentType: "application/json; charset=utf-8",
			dataType: "json",
			data: JSON.stringify({
				telNum: tel
