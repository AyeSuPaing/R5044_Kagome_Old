...
{
	// ポップアップ表示用クエリ
	static string WINDOW_KBN_PARAM = Constants.REQUEST_KEY_WINDOW_KBN + "=" + Constants.KBN_WINDOW_POPUP;

	/// <summary>
	/// ポップアップページチェック
	/// </summary>
	/// <returns></returns>
	protected void CheckPopupPage()
	{
		//------------------------------------------------------
		// ポップアップ画面切り替え処理
		//------------------------------------------------------
		...
		{
			if ((Request.Url.Query.IndexOf(WINDOW_KBN_PARAM) == -1)
				&& (Request.UrlReferrer.Query.IndexOf(WINDOW_KBN_PARAM) != -1))
			{
				// ポップアップ表示を行うためのクエリを追加してリダイレクト
				StringBuilder sbRedirectUrl = new StringBuilder();
				sbRedirectUrl.Append(Request.Url.PathAndQuery).Append((Request.Url.Query == "") ? "?" : "&").Append(WINDOW_KBN_PARAM);
