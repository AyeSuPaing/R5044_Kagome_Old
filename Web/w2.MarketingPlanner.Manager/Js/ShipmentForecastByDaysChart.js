...
function CreateShipmentForecastByDaysCharts(jsonValues, chartType, isViewNumber, xlabel, ylabel1, ylabel2) {
  var priceValues = [];
  var shipmentValues = [];
  var labelValues = [];
  for (var i = 0; i < jsonValues.length; i++) {
    priceValues[i] = (jsonValues[i]["total_order_price_subtotal"] < 0) ? 0 : jsonValues[i]["total_order_price_subtotal"];
    shipmentValues[i] = (jsonValues[i]["shipment_order_count"] < 0) ? 0 : jsonValues[i]["shipment_order_count"];
    labelValues[i] = jsonValues[i]["monthDay"];
  }
  ...
  maxDataValue = Math.max.apply(null, shipmentValues);
    ...
    tooltips: {
      callbacks: {
        label: function (tooltipItem) {
          var unit = (tooltipItem.datasetIndex == 0) ? "円" : "件";
