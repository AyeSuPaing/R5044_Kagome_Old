	...
	class Program
	{
		...
		private void ImportOrderMail(Constants.ExecTypes execType)
		{
			...
			foreach (var strMailFile in mailFiles)
			{
				...
				{
					htMails[strMailFile] = trReader.ReadToEnd();
				}
			}
			...
			using (SqlStatement sqlStatement = new SqlStatement("Mall", "GetMallSetingFromMallId"))
			{
				Hashtable htInput = new Hashtable();
				htInput.Add(Constants.FIELD_MALLCOOPERATIONSETTING_MALL_ID, StringUtility.ToEmpty(this.MailId));

				dvMallCooperationSetting = sqlStatement.SelectSingleStatementWithOC(sqlAccessor, htInput);
			}
			if (dvMallCooperationSetting.Count > 0)
			{
				strMallKbn = (string)dvMallCooperationSetting[0][Constants.FIELD_MALLCOOPERATIONSETTING_MALL_KBN];
			}
			else
			{
					...
					switch (strMallKbn)
					{
						...
						case Constants.FLG_MALLCOOPERATIONSETTING_MALL_KBN_RAKUTEN:
							...
							if (RakutenProcess.CheckMailImportPossible(subject, key))
							{
								...
								var rakutenProcess = new RakutenProcess(this.ShopId, key, mmMailMessage.Body, dvMallCooperationSetting[0]);
			...
			using (SqlAccessor sqlAccessor = new SqlAccessor())
			{
				...
				foreach (string strKey in htMallMails.Keys)
				{
					...
					try
					{
						...
						if (strMallKbn == Constants.FLG_MALLCOOPERATIONSETTING_MALL_KBN_RAKUTEN)
						{
							((RakutenProcess)htMallMails[strKey]).InsertRakutenOrder(sqlAccessor);
						}
						else if (strMallKbn == Constants.FLG_MALLCOOPERATIONSETTING_MALL_KBN_YAHOO)
						{
		...
		{
			var result = (execType == Constants.ExecTypes.Active);
			result &= (ex is RakutenApiCoopException);
			result &= (ex.Message.StartsWith(RakutenApiCoopException.ORDER_GET_ERROR));
			return result;
		}
