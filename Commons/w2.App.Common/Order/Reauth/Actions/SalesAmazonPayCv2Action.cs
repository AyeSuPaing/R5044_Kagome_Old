/*
=========================================================================================================
  Module      : 再与信アクション（Amazon Pay(CV2)売上確定）クラス(SalesAmazonPayCv2Action.cs)
 ･･･････････････････････････････････････････････････････････････････････････････････････････････････････
...
namespace w2.App.Common.Order.Reauth.Actions
{
	...
	/// </summary>
	public class SalesAmazonPayCv2Action : BaseReauthAction<SalesAmazonPayCv2Action.ReauthActionParams>
	{
		#region コンストラクタ
		/// <summary>
		/// コンストラクタ
		/// </summary>
		public SalesAmazonPayCv2Action(ReauthActionParams reauthActionParams)
			: base(ActionTypes.Sales, "Amazon Pay(CV2)売上確定", reauthActionParams)
		{
		}
		#endregion
		...
		private ReauthActionResult ExecSalesAmazonPay(OrderModel orderNew, decimal salesAmount)
		{
			try
			{
				var capture = new AmazonCv2ApiFacade().CaptureCharge(orderNew.CardTranId, salesAmount);
				var captureError = AmazonCv2ApiFacade.GetErrorCodeAndMessage(capture);
				if (capture.Success == false)
				{
					return new ReauthActionResult(
						...
						apiErrorMessage: LogCreator.CreateErrorMessage(captureError.ReasonCode, captureError.Message));
				}

				return new ReauthActionResult(
					true,
					orderNew.OrderId,
					CreatePaymentMemo(orderNew.OrderPaymentKbn, orderNew.PaymentOrderId, capture.ChargeId, orderNew.LastBilledAmount),
					cardTranId: capture.ChargeId,
					cardTranIdForLog: capture.ChargeId);
			}
		...
		{
			base.ReauthActionParams.OrderNew.PaymentOrderId = paymentOrderId;
			base.ReauthActionParams.OrderNew.CardTranId = cardTranId;
		}
		#endregion
