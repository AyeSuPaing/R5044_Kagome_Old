/*
=========================================================================================================
  Module      : BOTCHAN専用注文登録・更新(OrderUpdaterBotchan.cs)
  ･･･････････････････････････････････････････････････････････････････････････････････････････････････････
...
namespace w2.App.Common.Order.Botchan
{
	...
	/// BOTCHAN専用注文登録・更新
	/// </summary>
	public class OrderUpdaterBotchan
	{
		#region 定数
			...
			out string errorMessage)
		{
			...
			using (var accessor = new SqlAccessor())
			{
				...
				try
				{
					ExecuteUpdateOrder(doStockCooperation, doAlertOutOfStock, updateHistoryAction, accessor, out transactionName);
			...
			out string errorMessage)
		{
			...
			using (var accessor = new SqlAccessor())
			{
				...
				try
				{
					...
					// 更新
					var executeResult = ExecuteUpdateOrder(doStockCooperation, doAlertOutOfStock, updateHisoryAction, accessor, out transactionName);

					// 在庫切れ
					if (executeResult == ResultType.OutOfStock)
					{
						errorMessage = MessageManager.GetMessages(BotchanMessageManager.MessagesCode.PRODUCTSTOCK_OUT_OF_STOCK_ERROR.ToString());
					...
					if ((executeResult == ResultType.Success) && isRegisterCreditCard)
					{
							...
							&& (userCreditCard.CardDispName != Constants.CREDITCARD_UNREGIST_DEFAULT_DISPLAY_NAME))
						{
							...
							if (isSuccess == false)
							{
								errorMessage = MessageManager.GetMessages(BotchanMessageManager.MessagesCode.USERCREDITCARD_CANNOT_UPDATE_DISP_FLG_ERROR.ToString())
									.Replace("@@ 1 @@", userCreditCard.UserId)
									.Replace("@@ 2 @@", userCreditCard.BranchNo.ToString());
		...
		{
			// 注文情報
			var isSuccess = UpdateOrderInfo(updateHistoryAction, accessor, out transactionName)
				? ResultType.Success
				: ResultType.Fail;
			...
			if (Constants.W2MP_POINT_OPTION_ENABLED
				...
				&& this.User.IsMember
				&& (this.UserPoint != null)
				&& (this.UserPointOld != null))
			{
				isSuccess = UpdateOrderPointInfo(updateHistoryAction, accessor, out transactionName)
					? ResultType.Success
					: ResultType.Fail; ;
			}
			...
			out string errorMessage)
		{
			...
			else
			{
				if (reauthResult.ResultDetail != ReauthResult.ResultDetailTypes.Success)
				{
					errorMessage = reauthResult.ErrorMessages
						+ Environment.NewLine
						+ MessageManager.GetMessages(BotchanMessageManager.MessagesCode.EXTERNAL_PAYMENT_CANCEL_FAILED.ToString());
				}
		...
		/// <param name="transactionName">トランザクション名</param>
		/// <returns>成功したか</returns>
		private bool UpdateOrderPointInfo(UpdateHistoryAction updateHistoryAction, SqlAccessor accessor, out string transactionName)
		{
			transactionName = string.Empty;
			var isSuccess = true;
			if (this.Order.OrderPointUse != this.OrderOld.OrderPointUse)
			{
				// 利用ポイント変更
			...
			// 差異があるものだけ抽出してUPDATE
			var pointsForUpdate = this.UserPoint.Items
				.Where(p => (this.UserPointOld.GetUserPoint(p.PointKbnNo) != null)
					&& (p.Point != this.UserPointOld.GetUserPoint(p.PointKbnNo).Point));
			foreach (var userPoint in pointsForUpdate)
			{
				transactionName = "1-5-2-A.ユーザポイント情報(付与ポイント)UPDATE処理";
				isSuccess = UpdateUserPoint(
					userPoint,
					(userPoint.Point - this.UserPointOld.GetUserPointByPointKbnNo(userPoint.PointKbnNo).Point),
					updateHistoryAction,
					accessor);
			}

			// UserPointOldになくてUserPointにあるものをINSERT
			foreach (var newUserPoint in this.UserPoint.Items)
			{
				if ((newUserPoint.IsPointTypeComp
					&& (this.UserPointOld.UserPointComp.Length == 0))
						|| ((newUserPoint.IsPointTypeComp == false)
						&& (this.UserPointOld.GetUserPointByPointKbnNo(newUserPoint.PointKbnNo) == null)))
				{
					transactionName = "1-5-2-C.ユーザポイント情報(付与ポイント)INSERT処理";
					isSuccess = InsertUserPoint(newUserPoint, updateHistoryAction, accessor);
				}
			}
		...
		/// <param name="accessor">SQLアクセサ</param>
		/// <returns>成功したか</returns>
		private bool InsertUserPoint(UserPointModel userPoint, UpdateHistoryAction updateHistoryAction, SqlAccessor accessor)
		{
			var service = new PointService();
			var master = service.GetPointMaster().First(p => (p.PointKbn == userPoint.PointKbn));
			var point = new UserPointModel
			{
				...
				PointIncKbn = (userPoint.IsPointTypeComp == false)
					? Constants.FLG_USERPOINTHISTORY_POINT_INC_KBN_KBN_ORDER
					: string.Empty,
				Point = userPoint.Point,
				PointExp = master.IsValidPointExpKbn
					? (DateTime?)new DateTime(this.Order.OrderDate.Value.Year, this.Order.OrderDate.Value.Month, this.Order.OrderDate.Value.Day, 23, 59, 59, 997).AddYears(1)
					: null,
				Kbn1 = (userPoint.IsPointTypeComp == false) || (userPoint.IsLimitedTermPoint)
					? this.Order.OrderId
					: string.Empty,
				Kbn2 = string.Empty,
				Kbn3 = string.Empty,
			...
			// 履歴登録
			var userPoints = service.GetUserPoint(userPoint.UserId, string.Empty, accessor);
		...
		/// ユーザポイント情報更新
		/// </summary>
		/// <param name="userPointComp">ユーザ本ポイント情報</param>
		/// <param name="point">ポイント変更数</param>
		/// <param name="updateHistoryAction">更新履歴アクション</param>
		/// <param name="accessor">SQLアクセサ</param>
		/// <returns>成功したか</returns>
		private bool UpdateUserPoint(UserPointModel userPointComp, decimal point, UpdateHistoryAction updateHistoryAction, SqlAccessor accessor)
		{
			var service = new PointService();
			var userPoint = service.GetUserPoint(this.Order.UserId, string.Empty, accessor);
			var userUpdatePoint = userPoint.FirstOrDefault(a => a.PointKbnNo == userPointComp.PointKbnNo);
			...
			// 履歴登録
			var userBasePoint = userPoint.Where(a => a.IsBasePoint);
			var pointHistory = new UserPointHistoryModel
			{
				...
				PointExpExtend = UserPointHistoryModel.DEFAULT_POINT_EXP_EXTEND_STRING,
				UserPointExp = userBasePoint.Max(x => x.PointExp),
				Kbn1 = this.Order.OrderId,
				Kbn2 = string.Empty,
				Kbn3 = string.Empty,
				Kbn4 = string.Empty,
