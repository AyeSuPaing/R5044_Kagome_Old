/*
=========================================================================================================
  Module      : Facebook Catalog Api Logger (FacebookCatalogApiLogger.cs)
  ･･･････････････････････････････････････････････････････････････････････････････････････････････････････
...
namespace w2.App.Common.Facebook.Logger
{
	...
	/// 各種ファイルログを出力する
	/// </summary>
	internal class FacebookCatalogApiLogger : FileLogger
	{
		/// <summary>Facebook Catalog API log path</summary>
		private static string s_facebookCatalogApiLogPath = (Constants.PHYSICALDIRPATH_LOGFILE + Constants.FACEBOOK_CATALOG_API);
		...
		{
			if (Directory.Exists(s_facebookCatalogApiLogPath) == false)
			{
				Directory.CreateDirectory(s_facebookCatalogApiLogPath);
			}

			var today = DateTime.Now.ToString(Constants.DATE_FORMAT_SHORT);
			var filePath = string.Format(
				"{0}/{1}_{2}.{3}",
				s_facebookCatalogApiLogPath,
				Constants.FACEBOOK_CATALOG_API,
				today,
		...
		{
			if (Constants.FACEBOOK_CATALOG_API_LOGFILE_THRESHOLD < 0) return;

			var file = new FileInfo(filePath);
			if (file.Length < Constants.FACEBOOK_CATALOG_API_LOGFILE_THRESHOLD * 1024 * 1024) return;

			var fileName = string.Format("{0}_*", prefix);
			var pattern = string.Format("{0}_{1}.{2}$", prefix, "(?<number>[0-9]+)", suffix);
			var maxValue = Directory.GetFiles(s_facebookCatalogApiLogPath, fileName)
