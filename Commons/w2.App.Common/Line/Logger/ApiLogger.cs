...
namespace w2.App.Common.Line.Logger
{
	/// <summary>
	/// 各種ファイルログを出力する
	/// </summary>
	internal class ApiLogger : FileLogger
	{
		...
		{
			if (Directory.Exists(Constants.DIRECTORY_LINE_API_LOGFILEPATH) == false)
			{
				Directory.CreateDirectory(Constants.DIRECTORY_LINE_API_LOGFILEPATH);
			}

			var today = DateTime.Now.ToString("yyyyMMdd");
			var filePath = string.Format(
				"{0}{1}_{2}.{3}",
				Constants.DIRECTORY_LINE_API_LOGFILEPATH,
				Constants.LINE_API_LOGFILE_NAME_PREFIX,
				today,
			...
			try
			{
				...
				using (var mtx = new Mutex(false, filePath.Replace("\\", "_") + ".FileWrite"))
				{
					try
					{
						...
						var prefix = string.Format("{0}_{1}", Constants.LINE_API_LOGFILE_NAME_PREFIX, today);
		...
		public static void ForkFile(string filePath, string prefix, string suffix)
		{
			...
			if (Constants.LINE_API_LOGFILE_THRESHOLD < 0) return;
			if (file.Length < Constants.LINE_API_LOGFILE_THRESHOLD * 1024 * 1024) return;

			var fileName = string.Format("{0}_*", prefix);
			var pattern = string.Format("{0}_{1}.{2}$", prefix, "(?<number>[0-9]+)", suffix);

			var max = Directory.GetFiles(Constants.DIRECTORY_LINE_API_LOGFILEPATH, fileName)
		...
		public static string GetOutPut(HttpStatusCode code, string message, string param, string response)
		{
			...
			var details = string.Format(
				...
				param,
				response);
			switch (Constants.LINE_API_LOG_LEVEL)
			{
				case Constants.LINE_API_FLG_LOG_LEVEL_INFO:
					return status;

				case Constants.LINE_API_FLG_LOG_LEVEL_ERROR:
					return (code == HttpStatusCode.OK) ? status : status + details;
